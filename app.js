...
io.on('connection', function (socket) {
	...
	socket.on('joinroom', function (name) {
		...
		if (roomIndex == -1) {
			...
			io.sockets.emit('updateSocketRooms', 'namelist', targetArray[roomIndex].users);
		}
	});
	...
	socket.on('game', function (status, data) {
		updateSocketGame(status, data);
	});
	...
	function updateSocketGame(status, data) {
		let getRoomInfo = findSocketRoom(socket.data.room);
		let roomIndex = getRoomInfo.roomIndex;
		let targetArray = getRoomInfo.targetArray;
		...
		if (gameType == "bingobash") {
			if (status == 'playercards') {
				let userIndex = targetArray[roomIndex].users.findIndex(x => x.index === Number(socket.data.index));
				targetArray[roomIndex].users[userIndex].data = data;
				targetArray[roomIndex].users[userIndex].ready = true;

				let totalPlayers = 0;
				for (let n = 0; n < targetArray[roomIndex].users.length; n++) {
					if (targetArray[roomIndex].users[n].ready) {
						totalPlayers++;
					}
				}

				if (totalPlayers == targetArray[roomIndex].users.length) {
					io.sockets.in(targetArray[roomIndex].name).emit('updateSocketGame', 'ready', targetArray[roomIndex].users);
				}
				return;
			} else if (status == 'revealnumbers') {
				let postData = { numbers: data, users: targetArray[roomIndex].users };
				socket.broadcast.to(targetArray[roomIndex].name).emit('updateSocketGame', status, postData);
				return;
			} else if (status == 'updateusernumbers') {
				let userIndex = targetArray[roomIndex].users.findIndex(x => x.index === Number(socket.data.index));
				targetArray[roomIndex].users[userIndex].data = data;
				socket.broadcast.to(targetArray[roomIndex].name).emit('updateSocketGame', status, targetArray[roomIndex].users);
				return;
			} else if (status == 'bingo') {
				socket.broadcast.to(targetArray[roomIndex].name).emit('updateSocketGame', status, socket.data.username);
				return;
			}
		} else if (gameType == "snakeandladders") {
			if (status == 'start') {
		...
		io.sockets.in(targetArray[roomIndex].name).emit('updateSocketGame', status, data);
	}
});